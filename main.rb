...
parser = OptionParser.new do |opts|
  ...
  opts.on("-o", "--statout", "Path to options.tsv and vents_meta.tsv.") do |o|
    options[:output] = o
  end
...
#check for each vent in <input_dir> if it exists in init file
#print init_file
ARGV.each do |vent|
  if !Dir.exists?(vent)
...
ARGV.each do |vent|
 ...
 if !File.exists?(tax_file) || !File.exists?(func_file)
   puts "Profile file not found for #{vent}. Skipping this one."
   next
 end
...
Downstream.write_anosim_results(anosim_arr1,"#{out}/taxonomic_profile_")
Downstream.write_anosim_results(anosim_arr2,"#{out}/functional_profile_")


#stamp
cats.each do |cat|
 Downstream.stamp_run(funcprof_file_stamp,"#{out}/vents_meta.tsv",cat,"#{out}/stamp/stamp_funcprof_#{cat}.pdf")
 Downstream.stamp_run(taxprof_file_stamp,"#{out}/vents_meta.tsv",cat,"#{out}/stamp/stamp_taxprof_#{cat}.pdf")
end

...
#   `mkdir #{prepro_reads}/temp`
  `mkdir #{diamond}` if !File.directory?(diamond)
 Dir.glob("#{prepro_reads}/*fastq.gz") do |run|  
   run_nr = File.basename(run.sub('.fastq.gz',''))
   `diamond blastx -d /scratch/gi/coop/perner/metameta/pfam27 -q #{run} -a #{vent}/profiles/functional/diamond/#{run_nr}_matches`# -t #{prepro_reads}/temp`
  ...
  summary = combine(diamond,'diamond')
